...
fn main(){
    ...
    fn some_fn<'a, 'b>(first_str: &'a str, second_str: &'b str) -> &'a str { // here if we change first_str to second_str it gives error
        // error is data of lifetime mismatched
        first_str          
    }
    ...
    fn greater(i:&i32,j:&i32) -> i32 {
        if i> j {
            *i          /// here we returning a variable not reference   
            // by adding starts * they are references and we need * to get the values from variables which is refernece
            // the same programme but return is vaue instead of reference
        } else {
            *j          
        }
    }
